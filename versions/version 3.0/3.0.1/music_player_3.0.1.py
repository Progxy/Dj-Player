...
def resume_martingarrix():
    ...
    return go, long, time_translator()
...
def reset():
    ...
    global long, rt, simbol, go, in_use

    if (simbol == True and go == False):

        pygame.mixer.music.set_pos(long)

        finestra.after(int(rt), reset)

    elif (in_use == 1 and go == False):

        in_use = 0

        simbol = True

        return simbol, in_use

    elif(in_use == 0 and go == False): 
        ...
        return simbol, go, time_translator()
...
def end_loop():
    ...
    return in_use, destroyer()

def all_loop():

    global long, b_pmer, song_name2, b_pmer2, rt, go, in_use
    ...
    waiter_time = beat*1000

    rt = int(waiter_time)

    go = False

    return in_use, go, rt, reset()
...
def change_tempo():
    ...
    new_const = longer / int(len_new) + 1

    new_moment = long / new_const
    ...
    pygame.mixer.music.set_pos(new_moment)

    return song_name2, lenght, b_pmer2, long, time_translator()
...
destoyr = Button(finestra, text = "End Loop", command = end_loop)
destoyr.grid(row = 16, column = 3)
